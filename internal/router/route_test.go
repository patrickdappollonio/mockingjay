...
func TestRoute_MatchRequest_HTTPVerbs(t *testing.T) {
	...
	for _, verb := range verbs {
		t.Run("verb_"+verb, func(t *testing.T) {
			...
			match, gotMatch := route.MatchRequest(req)
			if !gotMatch {
				t.Errorf("MatchRequest() should match verb %s", verb)
				return
			}
			...
			// Test non-matching verb (use different verb)
			otherVerb := "POST"
			if verb == "POST" {
				otherVerb = "GET"
			}

			req2, err := http.NewRequest(otherVerb, "/test", nil)
			if err != nil {
				t.Fatalf("Failed to create request: %v", err)
			}
			...
			if gotMatch2 {
				t.Errorf("MatchRequest() should not match different verb %s when route expects %s", otherVerb, verb)
			}

			if match2 != nil {
				t.Error("MatchRequest() should return nil match for different verb")
			}
		})
	}
}
...
func TestRoute_matchesVerb(t *testing.T) {
	tests := []struct {
		name      string
		routeVerb string
		reqMethod string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			route := &Route{Verb: tt.routeVerb}
			if got := route.matchesVerb(tt.reqMethod); got != tt.want {
				t.Errorf("matchesVerb() = %v, want %v", got, tt.want)
			}
		})
	}
}
