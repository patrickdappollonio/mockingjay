	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				if route.Method != tt.routeConfig.GetNormalizedMethod() {
					t.Errorf("CompileRoute() Method = %v, want %v", route.Method, tt.routeConfig.GetNormalizedMethod())
				}
...
func TestCompiler_CompileRoutes_MultipleRoutes(t *testing.T) {
	...
	for i, route := range routes {
		...
		if route.Method != expectedConfig.GetNormalizedMethod() {
			t.Errorf("Route[%d] Method = %v, want %v", i, route.Method, expectedConfig.GetNormalizedMethod())
		}
