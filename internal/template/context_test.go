...
func TestNewTemplateContext_Basic(t *testing.T) {
	...
	if ctx.Headers["Content-Type"] != "application/json" {
		t.Errorf("NewTemplateContext() context Headers[Content-Type] = %v, want application/json", ctx.Headers["Content-Type"])
	}
	if ctx.Headers["X-User-Id"] != "12345" {
		t.Errorf("NewTemplateContext() context Headers[X-User-Id] = %q, want 12345", ctx.Headers["X-User-Id"])
	}

	// Verify query params are extracted
	if ctx.Query["debug"] != "true" {
		t.Errorf("NewTemplateContext() context Query[debug] = %v, want true", ctx.Query["debug"])
	}
	if ctx.Query["name"] != "world" {
		t.Errorf("NewTemplateContext() context Query[name] = %v, want world", ctx.Query["name"])
	}
...
func TestExtractHeaders(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			headers: http.Header{
				"Accept": []string{"text/html", "application/xml", "application/json"},
			},
			expected: map[string]string{
		...
		{
			name: "header with no values (empty slice)",
			headers: http.Header{
				"X-No-Values": []string{},
			},
			expected: map[string]string{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := extractHeaders(req)

			if len(result) != len(tt.expected) {
				t.Errorf("extractHeaders() length = %v, want %v", len(result), len(tt.expected))
				return
			}

			for key, expectedValue := range tt.expected {
				if actualValue, exists := result[key]; !exists || actualValue != expectedValue {
					t.Errorf("extractHeaders() result[%s] = %v, want %v", key, actualValue, expectedValue)
				}
			}
		})
	}
...
func TestExtractQueryParams(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			values: url.Values{
				"tags": []string{"golang", "testing", "http"},
			},
			expected: map[string]string{
				"tags": "golang, testing, http",
			},
		},
		{
		...
		{
			name: "query param with no values",
			values: url.Values{
				"no-values": []string{},
			},
			expected: map[string]string{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := extractQueryParams(tt.values)

			if len(result) != len(tt.expected) {
				t.Errorf("extractQueryParams() length = %v, want %v", len(result), len(tt.expected))
				return
			}

			for key, expectedValue := range tt.expected {
				if actualValue, exists := result[key]; !exists || actualValue != expectedValue {
					t.Errorf("extractQueryParams() result[%s] = %v, want %v", key, actualValue, expectedValue)
				}
			}
		})
	}
			...
			if result == nil {
				...
				case map[string]interface{}:
					...
					if mapResult, ok := result.(map[string]interface{}); ok {
						if _, hasError := mapResult["parse_error"]; hasError && tt.body == `{invalid json}` {
							// This is expected for invalid JSON
						} else if tt.body == `{"name":"test","value":123}` {
							if name, exists := mapResult["name"]; !exists || name != "test" {
...
func BenchmarkExtractHeaders(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = extractHeaders(req)
	}
}

func BenchmarkExtractQueryParams(b *testing.B) {
	values := url.Values{
		"debug":   []string{"true"},
		"format":  []string{"json"},
		"version": []string{"v1"},
