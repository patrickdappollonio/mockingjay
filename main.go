...
// createRootCommand builds and configures the root cobra command
func createRootCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceUsage:  true,
		SilenceErrors: true,
		Short:         "A flexible HTTP mock server with template support",
		Long: `Mockingjay is a lightweight, flexible HTTP mock server that allows you to define
mock API responses using YAML configuration files with Go template support.
...
func run(configFile, port string, debug, validateOnly bool) error {
	...
	if validateOnly {
		...
		fmt.Printf("   - All templates compiled successfully\n")
	...
	go func() {
		defer func() {
			if closeErr := watcher.Close(); closeErr != nil {
				logger.Error("failed to close config watcher", "error", closeErr)
			}
		}()
